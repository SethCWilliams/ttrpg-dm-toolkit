<script>
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const plotHook = await plotHookAPI.createPlotHook(campaignId, submitData);
            dispatch('created', { plotHook });
        } catch (err) {
            error = err.message || 'Failed to create plot hook';
        } finally {
            saving = false;
        }
    }
...
<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-700">
            <h2 class="text-2xl font-semibold text-white">Create New Plot Hook</h2>
        </div>
                ...
                <form on:submit|preventDefault={handleSubmit} class="space-y-6">
                    ...
                    <div class="flex justify-end space-x-3 pt-6 border-t border-gray-700">
                        ...
                        <button
                            ...
                            disabled={saving}
                        >
                            {saving ? 'Creating...' : 'Create Plot Hook'}
                        </button>
                    </div>
                </form>
            {/if}
