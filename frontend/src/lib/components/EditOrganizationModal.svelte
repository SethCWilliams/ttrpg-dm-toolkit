<script>
    ...
    let formData = {
        ...
        scope: organization.scope || 'local',
        headquarters_location_id: organization.headquarters_location_id || null,
        leader_npc_id: organization.leader_npc_id || null,
        goals: organization.goals || [],
        methods: organization.methods || [],
        resources: organization.resources || '',
        influence_level: organization.influence_level || 'minor',
        membership_size: organization.membership_size || 'small',
        notable_members: organization.notable_members || [],
        allies: organization.allies || [],
        enemies: organization.enemies || [],
        reputation: organization.reputation || '',
        status: organization.status || 'draft',
        visibility: organization.visibility || 'dm_only',
    ...
    async function loadAvailableOrganizations() {
        try {
            ...
            availableOrganizations = response.items.filter(org => org.id !== organization.id) || [];
        } catch (err) {
    ...
    function initializeFormData() {
        // Initialize form data with existing organization data
        Object.keys(organization).forEach(key => {
            if (organization[key] !== null && organization[key] !== undefined) {
                formData[key] = organization[key];
            }
        });
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const updatedOrganization = await organizationAPI.updateOrganization(campaignId, organization.id, submitData);
            dispatch('updated', { organization: updatedOrganization });
        } catch (err) {
...
<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-gray-800 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6 border-b border-gray-700">
            <h2 class="text-2xl font-semibold text-white">Edit Organization</h2>
        </div>
